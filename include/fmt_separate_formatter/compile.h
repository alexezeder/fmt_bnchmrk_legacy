...
template <typename Char, size_t N> struct fixed_string {
  ...
  constexpr fixed_string(const char* begin, const char* end) {
    copy_str<Char, const Char*, Char*>(begin, end, data);
  }
...
// A replacement field that refers to argument N and has format specifiers.
template <typename Char, typename T, int N, typename Formatter>
struct spec_field_with_fmt {
  using char_type = Char;
  Formatter fmt;
...
template <typename Char, typename T, int N, typename Formatter>
struct is_compiled_format<spec_field_with_fmt<Char, T, N, Formatter>>
    : std::true_type {};
...
template <typename Formatter> struct parse_specs_result {
  Formatter fmt;
  size_t end;
  int next_arg_id;
};
...
template <typename Char> struct parse_basic_format_specs_result {
  detail::dynamic_format_specs<Char> specs;
  size_t end;
};

template <typename Char, detail::fixed_string Str>
constexpr auto parse_basic_format_specs() {
  using ParseContext = basic_format_parse_context<Char>;
  using handler_type = detail::dynamic_specs_handler<ParseContext>;
  auto str = basic_string_view<Char>{Str.data, Str.size() - 1};
...
struct int_formatter {
  ...
  constexpr auto format(T value, FormatContext& ctx) const {
    using namespace detail;

    using uint_type = uint32_or_64_or_128_t<T>;
    uint_type abs_value(static_cast<uint_type>(value));
    char prefix[4]{};
    ...
    if constexpr (specs.alt) {
      prefix[prefix_size++] = '0';
      prefix[prefix_size++] = specs.type;
    }
    int num_digits = count_digits<4>(abs_value);
    return write_int(ctx.out(), num_digits, string_view(prefix, prefix_size),
                     specs, [num_digits, abs_value](auto it) {
...
template <typename Formatter> struct parse_result {
  Formatter fmt;
  size_t end;
};

template <typename T, typename Char, typename ParseContext>
constexpr auto get_default_formatter(ParseContext& ctx) {
  auto f = formatter<T, Char>();
  auto end = f.parse(ctx);
  return parse_result{f, end - ctx.begin()};
}

template <typename T, typename Char, detail::fixed_string Str,
          std::enable_if_t<std::is_integral_v<T>, int> = 0>
constexpr auto parse() {
  constexpr auto result = parse_basic_format_specs<Char, Str>();
...
template <typename T, typename Char, typename S, class = void>
struct has_parse : std::false_type {};

template <typename T, typename Char, typename S>
struct has_parse<T, Char, S,
                 std::void_t<decltype(parse<T, Char, S>(std::declval<S>()))>>
    : std::true_type {};

template <typename T, typename Char, typename S, size_t POS>
constexpr auto parse_specs(int next_arg_id) {
  constexpr auto str = static_cast<basic_string_view<Char>>(S());
    ...
    if constexpr (str[POS + 1] == '{') {
      ...
      if constexpr (str[POS + 1] == '}') {
        ...
        constexpr auto result =
            parse_specs<id_type, char_type, S, POS + 2>(ID + 1);
        return parse_tail<Args, result.end, result.next_arg_id>(
            spec_field_with_fmt<char_type, id_type, ID, decltype(result.fmt)>{
                result.fmt},
            format_str);
      }
    } else {
